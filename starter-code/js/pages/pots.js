...
function handleThemeSelection(e){
  document.querySelectorAll('[data-color]').forEach((color) => {
    if (e.currentTarget.dataset.color === color.dataset.color){
      const themeTarget = e.currentTarget.parentElement.dataset.dropdownMenu === 'add-pot-theme' ? 'add-pot-theme' : 'edit-pot-theme'
...
function createPotCardMarkup({theme, name, target, total}){
    document.querySelector('[data-modal="new-pot-modal"]').classList.remove('show-modal')
    return `<div class="card card--compact pots-card" data-pot="${name.trim()}">
          <div class="pots-card-heading">
            <div>
              <div class="color-dot" style="background-color: ${theme};" ></div>
              <h3>${name}</h3>
            </div>
            <div class="dropdown-container">
              <button data-dropdown-trigger="edit-pot-dropdown">
          ...
          <div class="pots-progress-wrapper">
            <div class="flex-between">
              <small>Total Saved</small>
              <h2>${formatAmount(total, false)}</h2>
            </div>
            <div class="bar pots-progress">
              <div class="bar--inner" style="background-color: ${theme}; width: ${(total/target) * 100}%"></div>
            </div>
            <div class="flex-between">
              <small>${((total/target) * 100).toFixed(2)}%</small>
              <small>Target of ${formatAmount(target, false)}</small>
            </div>
          </div>
          <div class="pots-card-btns">
...
function openDeletePotModal(btn){
  const clickedPotName = btn.closest('[data-pot]').dataset.pot
  const selectedItem = potState.pots.find((item) => item.name.trim() === clickedPotName)
  if (selectedItem.name === clickedPotName){
      potState.selectedPot = selectedItem;
...
function renderPotsPage(){
    ...
    document.querySelector('.pots-section').addEventListener('click', (e) => {
        ...
        const confirmDeleteBtn = e.target.closest('[data-delete-trigger="delete-pot"]')
        ...
        if (confirmDeleteBtn) deletePot(confirmDeleteBtn)
    })
